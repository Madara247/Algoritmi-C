...
/*
1) CaricaMisure:
Carica le misurazioni dal file 'datiprecipitazioni.txt' e le memorizza in un array di liste. L'array è composto da 10 elementi. 
...
int CaricaMisure(char* nome_file, PNodoListaMisura* misure) {
  ...
  while(fscanf(f,"%d %d %d %s",&mis,&a.codice,&a.valore,a.data)==4){
    AggiungiNodoLista(&misure[mis],a);
    m++;
  }
  fclose(f);
  return m;
...
int* TotaleSettori(PNodoListaMisura* misure, char* giorno) {
	// Implementare il corpo della funzione
  PNodoListaMisura lis;
  int* vett;
  vett=(int*)malloc(NUM_SETTORI*sizeof(int));
  ...
  for(int i=0;i<NUM_SETTORI;i++){
    lis=misure[i];
    while(lis!=NULL){
      if(strcmp(giorno,lis->info.data)==0){
        vett[i]=vett[i]+lis->info.valore;
      }
      lis=lis->next;
    }
  }
...
PNodoListaMisura ValoriOltreSoglia(PNodoListaMisura* misure, int soglia) {
	// Implementare il corpo della funzione
  PNodoListaMisura new=NULL;
  PNodoListaMisura lis;
  for(int i=0;i<NUM_SETTORI;i++){
    lis=misure[i];
    while(lis!=NULL){
      if(lis->info.valore>soglia){
        AggiungiNodoLista(&new,lis->info);
      }
      lis=lis->next;
    }
  }
...
int main(void) {
  ...
  do {
    ...
    switch (scelta) {
      ...
      printf("\n gli elementi caricati sono:%d",n);

		  break;

      case 2:
		  // acquisire il giorno
      printf("\n scrivi il giorno in cui vuoi effettuare una verifica:");
      scanf("%s",giorno);
      vet=TotaleSettori(vettore,giorno);
      for(int i=0;i<NUM_SETTORI;i++){
        printf("\n la somma di precipitazioni è:%d",vet[i]);
      }
