...
int CaricaAuto(char* nome_file, ListaModelli* pLM) {
   ...
   while(!feof(f)){
   if(fscanf(f,"%s %d %s",a.targa,&a.km_percorsi,b->modello)==3){
    b=AggiungiModello(pLM,b->modello);
    AggiungiAuto(&b->listaA,a);
    }
   }
   fclose(f);
  return 1;
...
ListaModelli CancellaModello (ListaModelli L, char *modelloDaEliminare) {
  ...
  ListaAuto b;
    ...
    if(a!=NULL){
      if(strcmp(modelloDaEliminare,a->modello)==0){
        b=cancellaLista(a->listaA);
        a->listaA=b;
      }else{
        CancellaModello(a->succ,modelloDaEliminare);
      }
...
int main(void)
{
 ...
 VisualizzaListaModelli(LM);
	...
	break;
      ...
      printf("\n inserisci un valore soglia di km:");
      scanf("%d",&soglia);
  ...
  la=CreaListaVetture(LM,soglia);
   ...
   printf("\n scrivi il nome del modello da cancellare:");
   scanf("%s",modello);
